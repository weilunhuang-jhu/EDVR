...
def main():
    ...
    t1 = time.perf_counter()
    print("time for initiating model L: " + str(t1-t0))
    ...
    t1 = time.perf_counter()
    print("time for loading model L: " + str(t1-t0))
    ...
    print("==============================")
    print("Super Res Server L is Listening:")
    flag = 'c' # c stands for continue
    while True:
        flag = socket.recv_string()
        if flag != 'c':
            socket.send_string("Closing model L is done")
            break
        else:
        ...
        if save_imgs:
            ...
            cv2.imwrite(osp.join(save_folder, '{}.png'.format(img_name + "_texture")), img_out)
            ...
            cv2.imwrite(osp.join(save_folder, '{}.png'.format(img_name + "_d_texture")), img_out_d4)
